...
if partner_erfassen:
    ...
    partner_geburtsdatum = st.date_input("Geburtsdatum Partner/Partnerin")
...
if staffelung_3a:
    staffelungsjahre_3a = st.multiselect("Bezug Jahre für 3a", [f"{alter+i}/{2024+i}" for i in range(58-alter, 71-alter)])

staffelung_bvg = st.checkbox("BVG gestaffelt beziehen?")
if staffelung_bvg:
    staffelungsjahre_bvg = st.multiselect("Bezug Jahre für BVG", [f"{alter+i}/{2024+i}" for i in range(58-alter, 71-alter)])
